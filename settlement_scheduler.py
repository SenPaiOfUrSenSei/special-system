#!/usr/bin/env python3
"""
Bridgr Dynamic Settlement Scheduler
===================================
Runs every 5 minutes to decide when to settle pools (USDT, USDC, ETH, SOL)
using a pre-trained HistGradientBoostingClassifier model and hard limits from .env.
"""
...
def get_db_connection():
    db_url = os.getenv("DATABASE_URL", "postgresql://postgres:postgres@127.0.0.1:5433/payment_portal")
    # Redirect container URL to localhost mapping when running on host
...
def get_binance_klines():
    ...
    # Return sensible default rates if Binance API fails
    return {
        "price_open": 3500.0,
        "price_high": 3505.0,
        "price_low": 3495.0,
...
def get_l2_metrics(conn, currency, start_time_epoch):
    ...
    for tx in txs:
        ...
        # Check if transaction involves the currency
...
# Load the historical buffer once on startup
print(f"[*] Warming up historical buffer from {HISTORICAL_FEATURES_PATH}...")
historical_buffer = load_historical_buffer(HISTORICAL_FEATURES_PATH)
print(f"[+] Loaded {len(historical_buffer)} rows for rolling windows.")
...
def check_and_settle():
    ...
    try:
        # Fetch current system pools
        cur = conn.cursor()
        cur.execute("SELECT currency, tracked_balance, exposure FROM system_pools")
        pools = cur.fetchall()
        cur.close()
        
        # 5 minutes window in seconds
        now_epoch = time.time()
        start_5m_epoch = now_epoch - 300
        
        # Fetch Binance klines (ETH/USDT)
        kline = get_binance_klines()
        
        # Fetch latest Etherscan / Blockchain gas & BTC price
        ...
        for pool in pools:
            ...
            # 1. Get Hard Limit from Environment
            ...
            # 3. Build ML Features for Inference
            ...
            # Append new row to buffer to compute lag & rolling window features
            new_row_df = pd.DataFrame([row_dict])
            combined_df = pd.concat([historical_buffer, new_row_df], ignore_index=True)
            
            # Compute lags & rolling
...
def main():
    ...
    try:
        ...
        print(f"[ERROR] Unexpected error during startup run: {e}")
    ...
    try:
        while True:
            ...
            try:
                ...
                print(f"[ERROR] Unexpected error during scheduler execution run: {e}")
    except KeyboardInterrupt:
        print("\n[+] Dynamic Settlement Scheduler stopped by user.")
